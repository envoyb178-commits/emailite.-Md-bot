...
const config = {
  ...
  pairNumber: "263716491962", // YOUR NUMBER
...
// DON'T DELETE SESSION - PREVENTS RESET ON RENDER
...
const askAI = async (q) => {
  try {
    ...
    return res.data.success || "Ask me anything!";
  }
};

// MUSIC - NO "SIGN IN" ERROR
const downloadSong = async (query) => {
  ...
  return { stream: stream.stream, title: video.title };
};

// EDUCATION TEXTBOOKS - ZIMSEC + CAMBRIDGE FREE
const getTextbook = async (subject, grade) => {
  ...
  const key = `${subject.toLowerCase()}_${grade.toLowerCase()}`;
...
global.commands = {
  ...
  video: { category: "DOWNLOAD", run: async (m, { sock, q }) => {
    ...
    try {
      ...
      await sock.sendMessage(m.key.remoteJid, { video: stream.stream, mimetype: 'video/mp4' }, { quoted: m });
    } catch {
  ...
  fb: { category: "DOWNLOAD", run: async (m, { sock, q }) => { await sock.sendMessage(m.key.remoteJid, { text: `📘 Facebook download coming soon` }, { quoted: m }); }},
  ig: { category: "DOWNLOAD", run: async (m, { sock, q }) => { await sock.sendMessage(m.key.remoteJid, { text: `📷 Instagram download coming soon` }, { quoted: m }); }},
  insta: { category: "DOWNLOAD", run: async (m, { sock, q }) => global.commands.ig.run(m, { sock, q }) },
  twitter: { category: "DOWNLOAD", run: async (m, { sock, q }) => { await sock.sendMessage(m.key.remoteJid, { text: `🐦 Twitter download coming soon` }, { quoted: m }); }},
  ...
  // EDUCATION - 13 + TEXTBOOKS
  maths: { category: "EDUCATION", run: async (m, { sock, q }) => {
    const url = await getTextbook('maths', q || 'zimsec_grade7');
  ...
  commerce: { category: "EDUCATION", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `💼 Commerce notes coming soon` }, { quoted: m }); }},
  english: { category: "EDUCATION", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `📖 English notes coming soon` }, { quoted: m }); }},
  geography: { category: "EDUCATION", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `🌍 Geography notes coming soon` }, { quoted: m }); }},
  history: { category: "EDUCATION", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `📜 History notes coming soon` }, { quoted: m }); }},
  science: { category: "EDUCATION", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `🔬 Science notes coming soon` }, { quoted: m }); }},
  shona: { category: "EDUCATION", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `🗣️ Shona notes coming soon` }, { quoted: m }); }},
  subjects: { category: "EDUCATION", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `📚 *Subjects*\n\nMaths, Physics, Biology, Chemistry, Commerce, English, Geography, History, Shona` }, { quoted: m }); }},
  syllabus: { category: "EDUCATION", run: async (m, { sock, q }) => { await sock.sendMessage(m.key.remoteJid, { text: `📋 *Syllabus*\n\n${q || 'ZIMSEC'}\nhttps://www.zimsec.co.zw/syllabi/` }, { quoted: m }); }},
  ...
  kickall: { category: "GROUP", run: async (m, { sock }) => { await sock.sendMessage(m.key.remoteJid, { text: `👢 Kick all coming soon` }, { quoted: m }); }},
  ...
  tag: { category: "GROUP", run: async (m, { sock, q }) => {
    if (!m.key.remoteJid.endsWith('@g.us')) return await sock.sendMessage(m.key.remoteJid, { text: `❌ Group only` }, { quoted: m });
    try {
      const meta = await sock.groupMetadata(m.key.remoteJid);
      const participants = meta.participants.map(p => p.id);
      await sock.sendMessage(m.key.remoteJid, { text: q || `📢 Tag`, mentions: participants }, { quoted: m });
    } catch {
      await sock.sendMessage(m.key.remoteJid, { text: `❌ Failed` }, { quoted: m });
    }
  }},
  ...
  // SECURITY - 11
  antilink: { category: "SECURITY", run: async (m, { sock }) => { global.settings.antilink = !global.settings.antilink; await sock.sendMessage(m.key.remoteJid, { text: `🔗 AntiLink: ${global.settings.antilink ? 'ON ✅' : 'OFF ❌'}` }, { quoted: m }); }},
  antibadword: { category: "SECURITY", run: async (m, { sock }) => { global.settings.antibadword = !global.settings.antibadword; await sock.sendMessage(m.key.remoteJid, { text: `🚫 AntiBadWord: ${global.settings.antibadword ? 'ON ✅' : 'OFF ❌'}` }, { quoted: m }); }},
  antidelete: { category: "SECURITY", run: async (m, { sock }) => { global.settings.antidelete = !global.settings.antidelete; await sock.sendMessage(m.key.remoteJid, { text: `🗑️ AntiDelete: ${global.settings.antidelete ? 'ON ✅' : 'OFF ❌'}` }, { quoted: m }); }},
  antispam: { category: "SECURITY", run: async (m, { sock }) => { global.settings.antispam = !global.settings.antispam; await sock.sendMessage(m.key.remoteJid, { text: `📵 AntiSpam: ${global.settings.antispam ? 'ON ✅' : 'OFF ❌'}` }, { quoted: m }); }},
  antitag: { category: "SECURITY", run: async (m, { sock }) => { global.settings.antitag = !global.settings.antitag; await sock.sendMessage(m.key.remoteJid, { text: `🏷️ AntiTag: ${global.settings.antitag ? 'ON ✅' : 'OFF ❌'}` }, { quoted: m }); }},
  ...
  // TOOLS - 32
  calc: { category: "TOOLS", run: async (m, { sock, q }) => { try { const r = eval(q); await sock.sendMessage(m.key.remoteJid, { text: `🔢 *Calculator*\n\n${q} = ${r}` }, { quoted: m }); } catch { await sock.sendMessage(m.key.remoteJid, { text: `❌ Invalid math` }, { quoted: m }); } }},
  ...
  hash: { category: "TOOLS", run: async (m, { sock, q }) => { const crypto = require('crypto'); await sock.sendMessage(m.key.remoteJid, { text: `🔐 *Hash*\n\nMD5: ${crypto.createHash('md5').update(q || 'text').digest('hex')}` }, { quoted: m }); }},
  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        console.log(`✅ 8-DIGIT PAIRING FOR: ${config.pairNumber}`);
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      // REMOVE PREFIX IF EXISTS - WORKS WITH OR WITHOUT
